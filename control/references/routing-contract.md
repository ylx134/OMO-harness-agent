...
| --- | --- | --- | --- | --- | --- | --- | --- |
| `J-L1` | `判断型` | `轻流程` | `check` through `acceptance-manager` inside `/control`; direct analysis only outside `/control` | question-boundary package | task.md when durable context matters; acceptance-report.md when `check` is used | answer itself or acceptance-report.md | checker inside `/control`; main thread only outside `/control` |
...
- `J-L1` may run on the main thread only when the user did not invoke `/control`.
- Inside `/control`, `J-L1` must dispatch `acceptance-manager`; the top-level orchestrator must not perform the review, analysis, or conclusion itself.

...
export function normalizeSignals(graph, signals = {}) {
  ...
  for (const step of Object.values(graph?.steps || {})) {
    for (const signalName of step?.dependsOnSignals || []) names.add(signalName);
    for (const signalName of step?.emitsSignals || []) names.add(signalName);
  }

  for (const signalName of Object.keys(signals || {})) names.add(signalName);
...
export function deriveSignalSchedulingState(graph, stepRuntime, signals) {
  ...
  for (const [stepId, step] of Object.entries(graph?.steps || {})) {
    ...
    if (TERMINAL_STEP_STATUSES.has(status) || activeStatuses().has(status)) continue;
    if (!pendingLikeStatuses().has(status)) continue;

    const stepDepsSatisfied = stepDependenciesSatisfied(stepRuntime, step.dependsOnStepIds || []);
    if (!stepDepsSatisfied) continue;

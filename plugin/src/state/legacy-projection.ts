...
function phaseForStepKind(kind) {
  if (kind === 'capability-hand') return 'capability-hand';
  if (kind === 'probe') return 'probe';
  if (kind === 'acceptance-closure') return 'acceptance-closure';
  return 'manager';
}

function deferredDispatchStateForStep(kind) {
  if (kind === 'capability-hand') return 'hand_in_progress';
  if (kind === 'probe') return 'probe_in_progress';
  if (kind === 'acceptance-closure') return 'acceptance_closure_in_progress';

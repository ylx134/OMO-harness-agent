// @ts-nocheck
import { activeStepsForState, boundedDispatchBudgets, hasExclusiveLiveStep, selectRunnableCapabilityHandSteps, selectRunnableProbeSteps, } from './concurrency.js';
...
export function selectNextDispatchPlan(state, options = {}) {
    if (!state?.mode || state.mode !== 'harness')
        return { dispatches: [], reason: 'not_harness' };
    ...
    if (budgets.hands > 0 && allowedKinds.has('capability-hand') && state.dispatchedManagers?.includes('execution-manager') && !hasExclusiveLiveStep(state)) {
        ...
        if (dispatches.length > 0)
            return { dispatches, reason: 'pending_capability_hand' };
        if (activeCapabilityHands.length > 0)
            return { dispatches: [], reason: 'active_dispatch_in_progress' };
    }
    ...
    if (budgets.probes > 0 && allowedKinds.has('probe') && state.dispatchedManagers?.includes('acceptance-manager')) {
        ...
        if (dispatches.length > 0)
            return { dispatches, reason: 'pending_probe' };

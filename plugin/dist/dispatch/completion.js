...
export function completeGraphStep(state, { stepId, source, completedAt = nowIso(), deliverablesSatisfied = false, } = {}) {
    const step = state?.graph?.steps?.[stepId];
    if (!step)
        return { state: recoverGraphRuntimeState(state), changed: false, reason: 'unknown_step' };

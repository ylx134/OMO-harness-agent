...
export function recoverGraphRuntimeState(state) {
    ...
    for (const [stepId, runtime] of Object.entries(stepRuntime)) {
        ...
        if (activeStatuses().has(normalized.status))
            activeStepIds.push(stepId);
        if (normalized.status === 'retryable_error')
            retryQueue.push(stepId);
    }
...
export function markStepInProgress(state, { stepId, sessionID = null, startedAt = nowIso() }) {
    if (!state?.graph?.steps?.[stepId])
        return recoverGraphRuntimeState(state);

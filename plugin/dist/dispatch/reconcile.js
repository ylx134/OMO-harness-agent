...
function shouldAutoAdvance(state, options) {
    if (options.forceDispatch)
        return true;
    return Boolean(state?.autopilotEnabled
        && !state?.activeDispatch
        && state?.deferredDispatchState !== 'retryable_error'
        && state?.currentPhase !== 'complete');
}
export async function reconcileRuntime(context) {
    const { workspace, state, source, appendPluginDebug, dispatchNextDeferredManager, dispatchNextDeferredHand, dispatchNextDeferredProbe, finalizeDeferredAcceptance, options = {}, } = context;
    ...
    if (shouldAutoAdvance(nextState, options)) {
        ...
        for (const dispatch of dispatchPlan.dispatches) {
            ...
            if (nextState?.deferredDispatchState === 'retryable_error')
                break;
        }
    }

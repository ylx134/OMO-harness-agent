...
|------|------------------|-----|----------|
| 代理数量 | 1 | 1 + subagent | 3（planner/executor/checker，复杂 harness 任务强制） |
...
对于高上下文、长流程、多阶段任务，`/control` 现在会强制要求 planner / executor / checker 子代理；如果子代理不可用，会明确报阻塞，不再静默降级为主 agent 单线程执行。

### `/control` 的边界：什么时候还是主 agent，什么时候必须起 subagent？

- **非常小的任务不会因为用了 `/control` 就强行起子代理。** 如果 Task Router Gate 判断这是纯问答、单行修改、文件操作、quick prototype，仍然会直接委托给 Sisyphus 主 agent 执行。
- **真正进入 harness 路由后，才会强制多代理。** 也就是一旦任务被分到 `F-M1`、`C-M1`、`A-M1` 或 `P-H1`，就必须由 planner / executor / checker 子代理真实参与。
- **现在改掉的是 silent degrade，不是小任务直做能力。** 以前的问题是：复杂任务已经进入 harness，却还能合法退化成主 agent 单线程自演三角；现在这条路被关掉了。
- **如果子代理不可用，Control 会显式报阻塞。** 它会把这件事记录成 route-blocking gap，而不是假装已经进入了完整 harness 流程。
...
|----------|----------|--------|------|
| 判断型 | quick | check | 轻流程 |
| 修复型 | deep | drive → check | 中流程（harness 路由禁止单线程降级） |
| 改造型 | deep | plan? → drive → check | 中流程（harness 路由禁止单线程降级） |
| 能力型 | ultrabrain | capability-planner → plan → drive → check | 中流程（harness 路由禁止单线程降级） |
...
### Q: 为什么这次 `/control` 没有自己继续单线程做完？

对于高上下文、多阶段的 harness 路由，`/control` 现在要求 planner / executor / checker 子代理真实参与。如果子代理不可用，Control 会把它记录为 route-blocking gap 并显式提示阻塞，而不是由主 agent 顺序扮演所有角色。
